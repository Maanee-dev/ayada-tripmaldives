...
import { Dumbbell, Gamepad2, Map, Trophy, CheckCircle2, Clock, Users, Zap, ChevronRight, ShoppingBag } from 'lucide-react';
...
export default function SportsRecreation({ resort }: SportsRecreationProps) {
  ...
  const handleAddToInquiry = (item: any) => {
    addItem({
      id: `sports-${item.title.toLowerCase().replace(/\s+/g, '-')}`,
  ...
  const facilities = [
    ...
    },
    {
      title: "Games Hall",
      time: "08:00 AM - 11:00 PM",
      description: "Our vibrant games hall is available at the Recreation Center for your enjoyment. It offers a wide range of entertainment including PS4 consoles, table tennis, billiards, table football, and darts, making it the perfect spot for friendly competition.",
      icon: Gamepad2,
      image: "https://images.unsplash.com/photo-1600677396412-71440aa7e30e?auto=format&fit=crop&q=80&w=1200"
    },
    {
      ...
      icon: Trophy,
      image: "https://images.unsplash.com/photo-1595435064212-c441721aaac1?auto=format&fit=crop&q=80&w=1200"
    }
  ];
      ...
      <div className="relative h-[60vh] md:h-[80vh] -mx-6 md:-mx-12 lg:-mx-16 -mt-16 md:-mt-24 mb-16 md:mb-24 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-stone-900 via-stone-900/20 to-transparent flex flex-col justify-end p-6 md:p-16 lg:p-24">
          <div className="max-w-7xl mx-auto w-full">
            ...
            <h1 className="text-4xl md:text-7xl lg:text-9xl font-serif text-white mb-6 leading-[0.9] tracking-tighter">AyClub</h1>
      ...
      <div className="max-w-7xl mx-auto px-4">
          ...
          {facilities.map((facility, index) => (
            <div key={index} className={`flex flex-col ${index % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-12 lg:gap-24 items-center`}>
              ...
              <div className="flex-1 space-y-6">
                ...
                <h2 className="text-3xl md:text-5xl font-serif leading-tight">{facility.title}</h2>
                <p className="text-stone-500 text-lg font-light leading-relaxed">
                  {facility.description}
                </p>
                <div className="pt-4 flex gap-4">
                  <button className="flex-1 bg-stone-900 text-white px-8 py-4 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-stone-800 transition-colors flex items-center justify-center gap-2 group">
                    Inquire Details <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
                  </button>
                  <button 
                    onClick={() => handleAddToInquiry(facility)}
                    className="px-6 py-4 bg-emerald-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2"
                  >
                    <ShoppingBag size={16} />
                    Add to Inquiry
                  </button>
        ...
        <div className="bg-stone-900 rounded-[3rem] p-8 md:p-20 text-white mb-32 overflow-hidden relative">
          ...
          <div className="relative z-10 max-w-4xl">
            ...
            <h2 className="text-4xl md:text-6xl font-serif mb-8 leading-tight">Jogging Tracks</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {joggingTracks.map((track, i) => (
                <div key={i} className="bg-white/5 backdrop-blur-sm border border-white/10 p-8 rounded-3xl group hover:bg-white/10 transition-colors">
                  <div className={`text-4xl font-serif mb-2 ${track.color}`}>{track.length}</div>
